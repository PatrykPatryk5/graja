    ...
    this.manager = new Manager({
      ...
      .on("trackError", (player, err) => {
        ...
        let errorEmbed = new MessageEmbed()
          .setColor("RED")
          .setTitle("Błąd odtwarzania!")
          .setDescription(`Nie udało się załadować utworu: \`${title}\``)
          .setFooter({
            text: "Ups! Coś poszło nie tak, ale to nie twoja wina!",
          });
            ...
            channel.send({
              embeds: [
                new MessageEmbed()
                  ...
                  .setDescription(`Rozłączony z <#${oldChannel}>`),
              ],
            });
          }
      ...
      .on(
        ...
        /** @param {EpicPlayer} player */ async (player, track) => {
          ...
          let trackStartedEmbed = this.Embed()
            .setAuthor({ name: "Teraz odtwarzane", iconURL: this.config.iconURL })
            .setDescription(
              `[${title}](${track.uri})` || "Brak opisu"
            )
            .addFields(
              {
                name: "Na żądanie",
        ...
        /** @param {EpicPlayer} */ async (player, track) => {
          ...
          if (autoQueue) {
            ...
            let queueEmbed = new MessageEmbed()
              .setColor(client.config.embedColor)
              .setAuthor({
                name: "Kolejka została zakończona",
                iconURL: client.config.iconURL,
              })
              .setFooter({ text: "Kolejka zakończona" })
              .setTimestamp();
            ...
            try {
              if (!player.playing && !twentyFourSeven) {
                setTimeout(async () => {
                  if (!player.playing && player.state !== "DISCONNECTED") {
                    let disconnectedEmbed = new MessageEmbed()
                      ...
                      .setDescription(
                        `Gracz został rozłączony z powodu braku aktywności.`
                      );
                    ...
                    client.warn(
                      `Player: ${player.options.guild} | Wciąż gra`
                    );
                  }
                ...
                client.warn(
                  `Player: ${
                    player.options.guild
                  } | Kolejka została zakończona [${colors.blue("WŁĄCZONE 24/7")}]`
                );
              } else {
                client.warn(
                  `Z graczem stało się coś nieoczekiwanego ${player.options.guild}`
                );
              }
              player.setNowplayingMessage(client, null);
            } catch (err) {
